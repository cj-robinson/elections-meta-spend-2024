...
```{r}
courier_list <- c('The Keystone', 'The \'Gander Newsroom', 'UpNorthNews', 'Cardinal & Pine', 'The Copper Courier', 'Iowa Starting Line', "The Nevadan", "Granite Post") 

meta_ads_raw %>% 
  filter(year(ad_delivery_start_date) %in% c(2020, 2022, 2024)) %>%
  ...
  distinct(page_name, network)
  mutate(courier_flag = ad_delivery_start_datez(network == "Courier Newsroom", "Courier Newsroom", "Other"),
         year = year(ad_delivery_start_date)) %>%
