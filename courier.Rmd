...
```{r}
meta_ads_raw %>% 
    filter(ad_creation_time >= '2024-07-01') %>% 
  ...
  group_by(week = floor_date(ymd(ad_delivery_start_time), unit = "week"), network) %>% 
  #sum spend (we'll use low spend as a minimum)
  summarize(upper_sum = sum(high_spend),
            low_sum = sum(low_spend),
            avg_sum = (sum(high_spend) + sum(low_spend))/2) %>% 
  ggplot(aes(x = week, y = avg_sum, fill = reorder(network, desc(low_sum)))) + 
  geom_bar(stat = "identity", position = position_dodge(preserve = "single")) +
  scale_y_continuous(label = scales::dollar_format(scale = .000001, suffix = "M")) +
  labs(title = "Courier Newsroom Political Ads Skyrocketed in October", 
       subtitle = "2024 weekly Meta ad spend by week advertiser started delivering ad", 
       fill = "News Network") +
  theme_minimal() +
  ...
  scale_fill_manual(values = c(
    ...
    "Star Spangled Media" = "#2F4F4F",  
    "Metric Media" = "#696969",
    "Star News" = "#C0C0C0", 
    "American Independent" = "#E5E4E2"
  ))  # Add annotation for the high bar on the right
...
meta_ads_raw %>% 
  ...
  labs(title = "Courier Concentrated Spending in Pennsyvania and North Carolina", 
       subtitle = "2024 October Courier Meta ad spend by page and month" ) +
